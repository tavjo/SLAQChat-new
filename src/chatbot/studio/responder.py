...
from src.chatbot.studio.helpers import get_resource, get_available_workers, update_available_workers, update_messages, default_resource_box
...
def responder_node(state: ConversationState = INITIAL_STATE) -> Command[Literal["data_summarizer", "response_formatter", "validator"]]:
    ...
    try:
        ...
        available_workers = state.available_workers if state.available_workers else None
        ...
        messages.append(HumanMessage(content=f"{response.justification}/n{messages[-1].content}", name="responder"))
        ...
        messages.append(HumanMessage(content=f"I am sorry, I am unable to retrieve the information because of the following error: {e}. Please try again later. You can visit the NExtSEEK website (nextseek.mit.edu) for more information.", name = "responder"))
